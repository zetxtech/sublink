  ...
  window.formData = function () {
    return {
      ...
      init() {
        ...
        const savedPredefinedRule = localStorage.getItem(
        ...
        if (
          ...
          !Array.isArray(this.selectedRules) ||
      ...
      restoreHistoryEntry(id) {
        const item = (this.conversionHistory || []).find(
          (entry) => entry?.id === id,
        );
        if (!item) return;
        ...
        if (typeof state.selectedPredefinedRule === "string")
          this.selectedPredefinedRule = state.selectedPredefinedRule;
        if (Array.isArray(state.selectedRules))
          this.selectedRules = state.selectedRules;
        if (typeof state.groupByCountry === "boolean")
          this.groupByCountry = state.groupByCountry;
        if (typeof state.includeAutoSelect === "boolean")
          this.includeAutoSelect = state.includeAutoSelect;
        if (typeof state.enableClashUI === "boolean")
          this.enableClashUI = state.enableClashUI;
        if (typeof state.externalController === "string")
          this.externalController = state.externalController;
        if (typeof state.externalUiDownloadUrl === "string")
          this.externalUiDownloadUrl = state.externalUiDownloadUrl;
        if (typeof state.configType === "string")
          this.configType = state.configType;
        if (typeof state.configEditor === "string")
          this.configEditor = state.configEditor;
      ...
      getSubconverterUrl() {
        ...
        return (
          origin + "/subconverter" + (queryString ? "?" + queryString : "")
        );
      },
