...
const LINK_FIELDS = [
  ...
  { key: "subconverter", labelKey: "subconverterConfigTitle", isSubconverter: true },
];
          ...
          <form {...{ "x-on:submit.prevent": "submitForm" }} class="space-y-8">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-md group">
              ...
              <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <span class="text-xs text-gray-400 italic">{t("UAtip")}</span>
                </div>
                <input
                  type="text"
                  x-model="customUA"
        ...
        >
          ...
          <div class="px-6 py-5">
            ...
            <textarea
              ...
              rows={4}
              x-bind:placeholder="`${window.location.origin}/clash?... 或 ${window.location.origin}/c/abc123`"
              ...
              <div class="relative group" key={field.key}>
                ...
                <div class="flex gap-2">
                  <input
                    type="text"
                    readonly
                    x-bind:value={field.isSubconverter ? `getSubconverterUrl()` : `shortenedLinks ? shortenedLinks?.${field.key} : generatedLinks?.${field.key}`}
                    ...
                    x-bind:class={field.isSubconverter ? "'text-gray-600 dark:text-gray-400 focus:ring-green-500'" : "shortenedLinks ? 'text-sky-600 dark:text-sky-400 font-semibold focus:ring-sky-500' : 'text-gray-600 dark:text-gray-400 focus:ring-green-500'"}
                  />
                  <button
                    type="button"
                    ...
                    x-on:click={field.isSubconverter ? `window.copyTextToClipboard(getSubconverterUrl()).then(() => { copied = '${field.key}'; setTimeout(() => copied = null, 200); }).catch(() => {})` : `window.copyTextToClipboard(shortenedLinks ? (shortenedLinks?.${field.key} ?? '') : (generatedLinks?.${field.key} ?? '')).then(() => { copied = '${field.key}'; setTimeout(() => copied = null, 200); }).catch(() => {})`}
                    x-bind:class={`{
